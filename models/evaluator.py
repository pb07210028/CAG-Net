...
class Evaluator():
    ...
    def __init__(self, args, dataloader):
        ...
        # define G
        self.net = define(args=args, gpu_ids=args.gpu_ids)
    ...
    def _load_checkpoint(self, checkpoint_name='best_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, checkpoint_name)):
            ...
            checkpoint['model_state_dict'] = state_dict_new
            '''
            
            if isinstance(self.net, torch.nn.DataParallel):
                msg = self.net.module.load_state_dict(checkpoint['model_state_dict'])
            else:
                msg = self.net.load_state_dict(checkpoint['model_state_dict'])
            print(msg)
    ...
    def _collect_running_batch_states(self):
        ...
        #if np.mod(self.batch_id, 100) == 1:
        vis_input = utils.make_numpy_grid(de_norm(self.batch['N']))
        vis_input2 = utils.make_numpy_grid(de_norm(self.batch['I']))
    ...
    def _forward_pass(self, batch):
        self.batch = batch
        img_in1 = batch['N'].to(self.device)
        img_in2 = batch['I'].to(self.device)
        self.G_pred = self.net(img_in1, img_in2)[-1]
